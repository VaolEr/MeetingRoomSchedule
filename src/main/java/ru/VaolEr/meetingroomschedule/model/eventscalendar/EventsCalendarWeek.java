    ...
    public void insertEvents(List<EventTo> eventsList) {
        for (EventTo eventTo : eventsList) {
            ...
            for (Date weekDate : weekDates) {
                Calendar weekDateCal = new GregorianCalendar();
                weekDateCal.setTime(weekDate);
                if (weekDateCal.get(Calendar.DAY_OF_YEAR) == eventCal.get(Calendar.DAY_OF_YEAR)) {

                    switch (eventCal.get(Calendar.DAY_OF_WEEK)) {
                        case 2:
                            mondayEvents.add(eventTo);
                            break;
                        case 3:
                            tuesdayEvents.add(eventTo);
                            break;
                        case 4:
                            wednesdayEvents.add(eventTo);
                            break;
                        case 5:
                            thursdayEvents.add(eventTo);
                            break;
                        case 6:
                            fridayEvents.add(eventTo);
                            break;
                        case 7:
                            saturdayEvents.add(eventTo);
                            break;
                    }
                }
            }
